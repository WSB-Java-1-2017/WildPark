...
import wildpark.world.WorldGen;
...
public class WildPark extends Application {
	...
	/**
	 * This defines the duration of a single step of Wild Park Duration Time. In
	 * each Step the value of wildParkTime attribute is increased by this value - in
	 * method makeWildParkTimeStep().
	 */
	...
	/**
	 * Make a single time step of Wild Park - add a single Wild Park Time unit
	 * defined in WILD_PARK_TIME_STEP_DURATION constnt
	 */
	public static void makeWildParkTimeStep() {
		...
		for (Animal animal : getAnimals()) {
			System.out.println(
					"-------------------------------------------------------------------------------------------------");
			System.out.println("TimeStep: " + wildParkTime.toHours()
					+ " --- WildPark.makeWildParkTimeStep(): foreach Animal: " + animal.getId());
		...
		// Check each dead animal (each Meat object) if it is still suitable for
		// consumption.
	...
	/**
	 * The collection of all animals in Wild Park. It also contains dead animals -
	 * animals are not removed from this collection after death but are treated as
	 * Meat == Food.
	 */
	...
	// final Background BACKGROUND_GREEN = new Background( new BackgroundFill(
	// Paint.valueOf( "0x00ff0088" ), new CornerRadii( 5 ), new Insets(1,1,0,0) ) );
	final Background BACKGROUND_GREEN = new Background(
			new BackgroundFill(Paint.valueOf("linear-gradient(from 0px 54px to 0px 0px, #44ff55 20%, 0x44ff5555 100%)"),
					new CornerRadii(5), new Insets(1, 1, 0, 0)));
	final Background BACKGROUND_OCEAN_BLUE = new Background(
			new BackgroundFill(Paint.valueOf("linear-gradient(from 0px 54px to 0px 0px, #2299ff 20%, 0x2299ff55 100%)"),
					new CornerRadii(5), new Insets(1, 1, 0, 0)));
	final Background BACKGROUND_POLAR_WHITE = new Background(
			new BackgroundFill(Paint.valueOf("linear-gradient(from 0px 54px to 0px 0px, #eeeeee 20%, 0xffffffff 100%)"),
					new CornerRadii(5), new Insets(1, 1, 0, 0)));
	final Background BACKGROUND_LAKE_BLUE = new Background(
			new BackgroundFill(Paint.valueOf("linear-gradient(from 0px 54px to 0px 0px, #66ddff 20%, 0x66ddff55 100%)"),
					new CornerRadii(5), new Insets(1, 1, 0, 0)));
	final Background BACKGROUND_RIVER_BLUE = new Background(
			new BackgroundFill(Paint.valueOf("linear-gradient(from 0px 54px to 0px 0px, #66ddff 20%, 0x66ddff55 100%)"),
					new CornerRadii(5), new Insets(1, 1, 0, 0)));
	// final Background BACKGROUND_MOUNTAIN_DARK = new Background( new
	// BackgroundFill( Paint.valueOf( "linear-gradient(from 0px 54px to 0px 0px,
	// #ff7722 20%, 0xff994455 100%)" ), new CornerRadii( 5 ), new Insets(1,1,0,0) )
	// );
	final Background BACKGROUND_MOUNTAIN_DARK = new Background(
			new BackgroundFill(Paint.valueOf("linear-gradient(from 0px 54px to 0px 0px, #ff9a31 20%, 0xffcd6455 100%)"),
					new CornerRadii(5), new Insets(1, 1, 0, 0)));
	final Background BACKGROUND_FOREST_GREEN = new Background(
			new BackgroundFill(Paint.valueOf("linear-gradient(from 0px 54px to 0px 0px, #11bb11 20%, 0x11cc2255 100%)"),
					new CornerRadii(5), new Insets(1, 1, 0, 0)));
	...
	public void start(Stage stage) {
		...
		try {
			...
			// Image(this.getClass().getResource("login.png").toString()));

			// Image applicationIcon = new
			// Image(getClass().getResourceAsStream("dukes_36x36.png"));
			// primaryStage.getIcons().add(applicationIcon);
			stage.getIcons().add(new Image("wildpark/favicon-32x32.png"));

		} catch (IllegalArgumentException e) {
			System.out.println(
					"Error loading favicon-32x32.png. Should be in \'wildpark\' directory together with WildPark.class file.");
			// System.exit(-1);
		}
		...
		menu2.getItems().addAll(menu2_AnimalList, menu2_AnimalSpeciesTotalWeight_LineChart,
				menu2_AnimalSpeciesTotalWeight_PieChart, menu2_AnimalCountChart, new SeparatorMenuItem(),
				menu2_GenerateWildParkArea);
		...
		// BorderPane( center , top , right, bottom, left )
		stage.setScene(new Scene(new BorderPane(new BorderPane(scrollPane, toolBar, null, null, slider), menuBar, null,
				statusBar, null)));
	...
	void initializeWildParkArea() {
		...
		for (int y = 0; y < WILD_PARK_AREA_HEIGHT; y++) {
			for (int x = 0; x < WILD_PARK_AREA_WIDTH; x++) {
				...
				// cellArray[x][y].setBackground( new Background( new BackgroundFill(
				// Paint.valueOf( "0x00FF00" ), CornerRadii.EMPTY, Insets.EMPTY ) ) );
				...
				areaCell.addEventHandler(MouseEvent.MOUSE_CLICKED, new EventHandler<MouseEvent>() {
					@Override
				...
				areaCell.addEventHandler(MouseEvent.MOUSE_EXITED, new EventHandler<MouseEvent>() {
					@Override
					public void handle(MouseEvent e) {
	...
	void newWildParkArea() {
		...
		int previousOceanRightRandom = WILD_PARK_OCEAN_AVERAGE_WIDTH - (int) (0.05 * WILD_PARK_AREA_WIDTH)
				+ random.nextInt((int) (0.10 * WILD_PARK_AREA_WIDTH)); // initial Ocean Right Edge

		int previousPolarLeftRandom = (int) (0.02 * WILD_PARK_AREA_WIDTH); // initial Polar Area left edge - 2% of Wild
																			// Park Width
		int previousPolarRightRandom = previousPolarLeftRandom + random.nextInt(WILD_PARK_POLAR_AREA_AVERAGE_WIDTH);

		int previousLakeLeftRandom = WILD_PARK_OCEAN_AVERAGE_WIDTH + (int) (0.20 * WILD_PARK_AREA_WIDTH); // initial
																											// lake left
																											// edge -
																											// from
																											// Ocean
		...
		int previousMountainLeftRandom = WILD_PARK_OCEAN_AVERAGE_WIDTH + (int) (0.60 * WILD_PARK_AREA_WIDTH); // initial
																												// lake
																												// left
																												// edge
																												// -
		...
		for (int y = 0; y < WILD_PARK_AREA_HEIGHT; y++) {
			// Ocean right limits
			currentOceanRightRandom = previousOceanRightRandom - 1 + random.nextInt(3); // -1, żeby różniło się w lewo o
																						// 1 lub w prawo o 1
			...
			if (y == WILD_PARK_POLAR_AREA_HEIGHT - 2)
				currentPolarRightRandom = currentPolarLeftRandom + 10;
			if (y == WILD_PARK_POLAR_AREA_HEIGHT - 1)
				currentPolarRightRandom = currentPolarLeftRandom + 5;
			...
			if (y == 11)
				currentLakeRightRandom = currentLakeLeftRandom + 5;
			if (y == 12)
				currentLakeRightRandom = currentLakeLeftRandom + 10;
			if (y == 10 + WILD_PARK_LAKE_HEIGHT - 2)
				currentLakeRightRandom = currentLakeLeftRandom + 10;
			if (y == 10 + WILD_PARK_LAKE_HEIGHT - 1)
				currentLakeRightRandom = currentLakeLeftRandom + 5;
			...
			if (y == 11)
				currentMountainRightRandom = currentMountainLeftRandom + 5;
			if (y == 12)
				currentMountainRightRandom = currentMountainLeftRandom + 10;
			if (y == 10 + WILD_PARK_MOUNTAIN_HEIGHT - 2)
				currentMountainRightRandom = currentMountainLeftRandom + 10;
			if (y == 10 + WILD_PARK_MOUNTAIN_HEIGHT - 1)
				currentMountainRightRandom = currentMountainLeftRandom + 5;
			...
			if (y == 0)
				currentForestRightRandom = currentForestLeftRandom + 20; // Real initial width of the Forest
			// if( y==24 ) currentForestRightRandom = currentForestLeftRandom+7;
			if (y == 0 + WILD_PARK_FOREST_HEIGHT - 2)
				currentForestRightRandom = currentForestLeftRandom + 7;
			if (y == 0 + WILD_PARK_FOREST_HEIGHT - 1)
				currentForestRightRandom = currentForestLeftRandom + 4;
			...
			for (int x = 0; x < WILD_PARK_AREA_WIDTH; x++) {
				...
				if (x > currentPolarLeftRandom && x < currentPolarRightRandom && y >= 0
						&& y < WILD_PARK_POLAR_AREA_HEIGHT) {
					areaCell.setCellType(CellType.POLAR_AREA);
					areaCell.setBackground(BACKGROUND_POLAR_WHITE); // jezioro
				}

				if (x > currentLakeLeftRandom && x < currentLakeRightRandom && y > 10
						&& y < 10 + WILD_PARK_LAKE_HEIGHT) {
					areaCell.setCellType(CellType.LAKE);
					areaCell.setBackground(BACKGROUND_LAKE_BLUE); // jezioro
				}

				if (x > currentMountainLeftRandom && x < currentMountainRightRandom && y > 10
						&& y < 10 + WILD_PARK_MOUNTAIN_HEIGHT) {
					areaCell.setCellType(CellType.MOUNTAIN);
					areaCell.setBackground(BACKGROUND_MOUNTAIN_DARK);
				}

				if (x > currentForestLeftRandom && x < currentForestRightRandom && y >= 0
						&& y < 0 + WILD_PARK_FOREST_HEIGHT) {
		...
		for (int x = 0; x < WILD_PARK_AREA_WIDTH; x++) {
			if (previousRiverY == -1) { // untill ocean
				...
				else if (currentRiverYRandom > 11 + WILD_PARK_LAKE_HEIGHT)
					currentRiverYRandom -= 2;
	...
	void populateWildPark() {
		...
		/*
		 * WildParkAreaCell areaCell = new WildParkAreaCell("Cell1"); for( int i=0; i<5;
		 * i++ ) { Animal bat = new InsectEatingBat( areaCell, false ); }
		 */

		// for( int i=0; i<LION_COUNT; i++ ) {
		// Animal lion = new Lion( new LionSpecification(), new WildParkAreaCell(
		// CellType.LAKE ), false );
		// addAnimal( lion );
		// }

		// for( int i=0; i<LEOPARD_COUNT; i++ ) {
		// Animal leopard = new Leopard( new LeopardSpecification(), new
		// WildParkAreaCell( CellType.DESERT ), false );
		// addAnimal( leopard );
		// }

		// // for( int i=0; i<CROCODILE_COUNT; i++ ) {
		// // Animal crocodile = new Crocodile( new CrocodileSpecification(), new
		// WildParkAreaCell( CellType.LAKE ), false );
		// // getAnimals().add( crocodile );
		// // }

		// for( int i=0; i<POLAR_BEAR_COUNT; i++ ) {
		// Animal polarBear = new PolarBear( new PolarBearSpecification(), new
		// WildParkAreaCell( CellType.LAKE ), false );
		// addAnimal( polarBear );
		// }
		//
	...
	private void GenerateWildParkArea() {
		WorldGen.SEED = new Random().nextLong();
		Image img = WorldGen.generate();
	...
	void addUIEventListeners() {
		stage.addEventFilter(MouseEvent.ANY, e -> {
			...
			// stage.toFront();
			// System.out.println( "MOUSE_ANY on main - " + e.getEventType() + " - SceneX:Y:
		...
		// Resets Wild Park life - The new terrain is randomly generated and life starts
		// from the begining.
		...
		toolBarButton_Open.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent e) {
				...
				// Opens the file explorer window to find the Wild Park Log File saved in
				// previous sessions.
		...
		toolBarButton_Save.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent e) {
				...
				// Open Wild Park Save As dialog box or just save the Park if the file name is
				// already known
		...
		toolBarButton_Reset.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent e) {
				...
				// Resets Wild Park life - but does not change the map. Life starts from the
				// begining.
				clearWildPark();
				populateWildPark();
			}
			...
			public void handle(ActionEvent e) {
				...
				if (result.isPresent()) {
					if (result.get() == "Animal List") {
						ReportAnimals ra = new ReportAnimals(getAnimals(), "Animal List"); // Just as-is but working
																							// report list
						try {
							ra.show();
						} catch (Exception e1) {
					...
					/*
					 * System.out.println("Your choice: " + result.get()); Stage stage2 = new
					 * Stage(); TextArea textArea = new TextArea();
					 * textArea.setText("Przykladowa tresc raportu"); Scene scene2 = new Scene(new
					 * BorderPane(textArea)); stage2.setScene(scene2); stage2.show();
					 */
				}
