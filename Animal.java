...
extends Meat 
{
	...
	public void performTimeStep() {
		...
		if(getAnimalState().isAlive==true) {

			float energyPercentLoose = 0;
			try {
				energyPercentLoose = 100 / (getAnimalSpeciesSpecification().getMAX_STARVING_DAYS_BEFORE_DEATH() * 24);
			...
			getAnimalState().useEnergy(energyPercentLoose);

			move(WildPark.getWildParkTimeStepDuration());

			// Loose energy on move
			getAnimalState().useEnergy(energyPercentLoose * 3);

			getAnimalState().hoursSinceLastMeal = getAnimalState().hoursSinceLastMeal.plus(WildPark.getWildParkTimeStepDuration());

			if (getAnimalState().getEnergyPercent() <= 0) {
				die();
			} else if (getAnimalState().energyPercent < getAnimalSpeciesSpecification().getHUNGER_ENERGY_PERCENT()) { //Zaczyna chudnąć
				reduceWeight(getAnimalState().getWeight() * energyPercentLoose / 100); // chudnie procentowo tyle, co traci energii
